...
# Error loading "C:\Users\...\site-packages\torch\lib\shm.dll" or one of its dependencies.
# Usage:  python fixNvPe.py --input=C:\Users\...\site-packages\torch\lib\*.dll (remember run with administrator)
...
def main(args):
    ...
    for file in glob.glob( args.input, recursive=args.recursive ):
        ...
        if len(nvbSect) == 1:
            ...
            if size > 0: #(writable or aslr) and size > 0:
                print("- Modifying DLL")
                if args.backup:
                    bakFile = f"{file}_bak"

# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 09:42:19 2023
...
class TSCNet(nn.Module):
    ...
    def forward(self, x): 
        ...
        x_mag = torch.sqrt(x_real ** 2 + x_imag ** 2).unsqueeze(1)
        x_phase = torch.angle(torch.complex(x_real, x_imag)).unsqueeze(1)
        
        x_in = torch.cat([x_mag, x], dim=1)
        ...
        mask = self.mask_decoder(out_5) #the magnitude scale paramaeter  
        mask_real, mask_imag = mask*x_mag*torch.cos(x_phase), mask*x_mag*torch.sin(x_phase)

        complex_out = self.complex_decoder(out_5) #the denoise value
        complex_real, complex_imag = complex_out[:, 0, :, :].unsqueeze(1), complex_out[:, 1, :, :].unsqueeze(1)
        
        final_real = mask_real + complex_real
        final_imag = mask_imag + complex_imag

# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 15:54:08 2023
...
def enhance_one_track(
    ...
    #Pre-Processing: Round and cut audio into sections ------------------------
    frame_num = int(np.ceil(length / 1000))
    padded_len = frame_num * 1000
    ...
    noisy_spec = torch.stft(
        noisy, 
        n_fft, hop, window=torch.hamming_window(n_fft).to(device), onesided=True
    )
    ...
    #Post-Processing: Combine sections of audio -------------------------------
...
def evaluation(model_path, noisy_dir, clean_dir, save_tracks, saved_dir):
    ...
    for audio in tqdm(audio_list):
        ...
        if not os.path.exists(clean_path) or len(est_audio) != length:
            metrics = -1
        else:
